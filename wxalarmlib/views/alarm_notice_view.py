...
class AlarmNoticeView(wx.Dialog):
    WINDOW_WIDTH = 370
    WINDOW_HEIGHT = 370

    def __init__(self, parent, element, status_element, close_event):
    ...
    def GetMinSnooze(self):
        return self.minSnooze

    def ShowModal(self):
        super(AlarmNoticeView, self).ShowModal()
    ...
    def InitializeComponents(self, parent):
        ...
        # ------------------------------------------------------------------
        # ListCtrl
        listctrl.InsertColumn(0, "行動候補", wx.LIST_FORMAT_LEFT, 350)
    ...
    def OnClickOption(self, event):
        ...
        message = "「%s」を実行しますか？" % opt[opt_title].strip()
    ...
    def SetWindowValue(self):
        ...
        command = AlarmStatusElement.OPT_COMMAND
        type_snooze = AlarmStatusElement.COMMAND_TYPE_SNOOZE
        opt_title = AlarmStatusElement.OPT_TITLE
        ...
        time = base_time
        text = "「" + title + " (" + time + ")」の通知です。"
        self.title.SetLabelText(text)

        # link to section
        link = status_element.GetFieldBy(AlarmElement.FIELD_LINK)
        section = self.element.GetLinkByName(link)

        # judge time
        judge_time = status_element.JudgeTime()
        ...
        for opt in options:
            # set min snooze
            if opt[command] == type_snooze:
                if self.minSnooze is None:
                    self.minSnooze = opt
                elif self.minSnooze[append_time] > opt[append_time]:
                    self.minSnooze = opt
    ...
    def OnCloseWindow(self, event):
        ...
        opt = self.minSnooze
        if opt is not None:
            message = "「%s」で作業を開始します" % opt[opt_title].strip()
            self.selectOpt = opt
            dialog = wx.MessageDialog(None, message, title, style=style)
            dialog.ShowModal()
